...
- **Cascade.** UA / author / inline ladder with `!important` inversion. CSS-faithful specificity. Interaction pseudo-classes (`:hover`, `:focus`, `:checked`, `:indeterminate`, `:open`, `:disabled`, `:enabled`, `:valid`, `:invalid`, `:required`, `:optional`, …). Pseudo-elements (`::before`, `::after`, `::selection`, `::backdrop`). `content` property. Custom properties.
...
Open polish items (no fixed milestone): `:focus-visible`, `::placeholder` / `:placeholder-shown`, undo/redo coalescing, blinking caret, whitespace normalization in clipboard serialization. Open debt is tracked in [`specs/TECH_DEBT.md`](specs/TECH_DEBT.md).
