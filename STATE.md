...
**Shipped:** **0.2.0 — live on crates.io (2026-06-02)** (all five crates; tag `v0.2.0`; headline: BFC, native ARIA tree, `calc()`, event surface bundle, multi-slot stylesheets, layered border model), followed by a run of **divergent `rdom-tui`-only patch releases 0.3.0 → 0.3.11**, most driven by the first real downstream consumer, **`rdom-virtualtable`**, exercising the substrate: substrate-honesty (0.3.0), focus/`:where()`/drop_subtree fixes (0.3.1–0.3.3, 0.3.10), table column-sizing (0.3.5–0.3.6), layout stale-state + half-block borders (0.3.7–0.3.9), and **drag-autoscroll** (0.3.11). Full ledger in the Semver release track + dated entries below.

**In flight (landed on `main`, pending the next `rdom-tui` bump → 0.3.12):** **drag-autoscroll robustness + text-selection precision** — five substrate fixes plus the sticky-container autoscroll engine, all surfaced by interactive testing of the `selectable_text` showcase demo (empty-space caret snap, `user-select:none` highlight/collapse, autoscroll stuck when the anchor scrolled out, and the sticky drag-scroll container + banded edge zone that is the durable design). See "2026-06-05 — drag-autoscroll robustness + selection precision" below.

**Status:** 0.2.0 milestone **closed** (M1–M9 below). The substrate is stable; active work is consumer-driven hardening (this session) + the `rdom-virtualtable` `SCROLL-SINGLE-OWNER-1` consumer refactor (tracked in that repo's STATE).
...
### 2026-06-05 — drag-autoscroll robustness + selection precision (prepared as 0.3.12)
...
Ships as **`rdom-tui` 0.3.12** (divergent bump; `rdom-core` unchanged this round). Follow-ups tracked: textarea builtin as a `user-select: contain` selection root; scroll chaining.
