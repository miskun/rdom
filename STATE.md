...
**Known limitation (tracked, not blocking):** the tree built-in has no scroll-into-view, so arrowing the cursor past the sidebar fold doesn't auto-scroll the `overflow: auto` sidebar (mouse wheel still scrolls). Categories ship expanded; all ~30 nav rows exceed a typical sidebar height. Same family as the deferred large-tree virtualization follow-up from TREE-1.
