...
**Latest:** **`rdom-tui` 0.3.14 (released 2026-06-06; tag `rdom-tui-v0.3.14`)** — `position_at` no longer snaps out of a `user-select: none` region (see CHANGELOG). **Active program (2026-09-21 →): [`specs/HARDENING-2026-09.md`](specs/HARDENING-2026-09.md)** — full-project review, entry-by-entry DIVERGENCES / TECH_DEBT audit, and the batched paydown (rdom-core → style/css → tui → parser). Batch 1 (rdom-core) in progress.

**Status:** 0.2.0 milestone **closed** (M1–M9 below). The 0.3.x line was consumer-driven hardening for `rdom-virtualtable`. The 2026-09 review found the substrate's invariants are not all held (stale `NodeId` aliasing, inverted `compareDocumentPosition` bits, panic-poisoned observers, timer API panics outside `handle_event`, text-only scroll containers that cannot scroll, CSS decimal misparse, at-rules swallowing rules) and that both contract docs drifted from the code. Fixing that is the current focus; the next releases are 0.4.0 across the changed crates.
