//! HTML §13.5 named character references — GENERATED, do not edit.
//!
//! Source: <https://html.spec.whatwg.org/entities.json> (2231 entries,
//! 106 legacy names without `;`). Regenerate with
//! `tools/gen_entities.py`. Keys omit the leading `&` and are sorted
//! bytewise; `parser::named_reference` binary-searches them.

/// `(name, replacement)` pairs, sorted by `name`. Names that end in
/// `;` are the modern form; the others are the legacy no-semicolon
/// references that HTML still decodes in text (with the attribute
/// caveat in §13.2.5.73).
#[rustfmt::skip]
pub(crate) static NAMED_REFERENCES: [(&str, &str); 2231] = [
    ("AElig", "\u{C6}"),
    ("AElig;", "\u{C6}"),
    ("AMP", "&"),
    ("AMP;", "&"),
...
/// Longest table name, in bytes (bounds the prefix scan).
pub(crate) const LONGEST_NAME: usize = 32;

/// Longest legacy (no `;`) name, in bytes: bounds the prefix loop for
/// references that did not match with a semicolon.
pub(crate) const LONGEST_LEGACY_NAME: usize = 6;
