...
- **`M5-MARGIN-1` — RESOLVED (2026-05-25).** CSS 2.1 §8.3.1 vertical margin collapse implemented in `layout_pass/block.rs` (BFC-1 Phase 5): adjacent-sibling collapse (`max(positives) + min(negatives)`), parent-first-child / parent-last-child collapse with new-BFC + padding + border blockers (uses the cascaded `establishes_new_bfc` field from Phase 1), and empty-block collapse-through. Out-of-flow siblings preserve adjacency (filtered before run partitioning). Follow-up `BFC1-MARGIN-COLLAPSE-UPWARD-1`: the merged margin from a parent-child collapse currently surfaces only locally — the parent's own outer position doesn't yet propagate the larger merged value to its grandparent. Visible only in nested-margin chains where the deeper child's margin would otherwise dominate; matters less for the common chrome shapes.
- **`BFC1-MARGIN-COLLAPSE-UPWARD-1` — Parent-child margin collapse doesn't propagate the merged value upward.** When a parent collapses its `margin-top` with its first in-flow block child, CSS 2.1 §8.3.1 says the *merged* margin (max of parent.mt + child.mt) surfaces at the parent's outer top, which then participates in collapse with the grandparent's accumulator. rdom's Phase 5.2 v1 just suppresses the child's margin-top contribution inside the parent — the parent's own margin-top is whatever its parent already placed it at, so the merged value isn't recomputed. Symmetric for bottom. For deeply-nested margin chains (uncommon shape in TUI chrome) the layout will under-margin compared to a real browser by the difference of the larger child margin. Fix requires a "what's my outer top margin" peek that recurses into the first child before placing the parent.
...
- **`BFC-1` — No block formatting context; everything lays out as flex.** `display: block` (the UA default for `<div>` / `<h1>` / `<p>` / `<section>` / most HTML block-level elements) is parsed and stored but has no distinct layout pass — block children of any container go through the flex algorithm regardless. The web-faithful behavior is: BFC children stack at their natural heights in document order, no shrink, no main-axis distribution; the container overflows below its content box if children don't fit. Real consumers writing standard HTML expect this. The current shape forces every author to either opt into `display: flex` deliberately or accept that "block" tags get flex behavior. Closing `M5-MIN-CONTENT-1` made the discrepancy invisible in practice for most content (items don't disappear, they overflow cleanly) but doesn't bridge the architectural gap. Real fix: a separate block layout pass that handles `display: block` containers and their in-flow children, with margin collapsing (`M5-MARGIN-1`) folded in. Substantial substrate work; probably its own post-0.2.0 milestone. Pay down when "rdom feels right to a CSS author" requires it (likely when downstream consumers start writing real HTML pages rather than flex-styled chrome).
...
- **`IFC-MIXED-TEXT-INLINEBLOCK-1` — Text nodes alongside an inline-block sibling don't render.** A container whose children are a mix of raw text + `display: inline-block` element (the prototypical case: `<p>text <button>btn</button> text</p>`) escapes IFC mode (the inline-block disqualifies the predicate per the M8 fix in `layout_pass/ifc.rs::is_ifc_block`) and falls into the regular flex layout, which treats text nodes as non-element children and gives them no flex slot. The button paints with its UA chrome; the surrounding text vanishes. Residual gap after the IFC-inline-block pseudo fix landed in M8. Proper fix: anonymous-inline-box generation around adjacent text + inline-block content, so the IFC packer can lay out both. Substantial substrate work; deferred until a real consumer needs the pattern. Workaround: wrap the text in `<span>` so the container is fully element-structured (the inline-block still escapes IFC, so the span ends up stacked under flex), or stack inline-block items on their own block lines.
