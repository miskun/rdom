...
- **`SHRINK-CLEANUP-1` — Redundant `flex-shrink: 0` declarations across showcase demos.** Before `M5-MIN-CONTENT-1` landed, demo + chrome CSS sprinkled `flex-shrink: 0` on ~60 fixed-height children to dodge the silent-vanish bug. Those declarations are now no-ops (the substrate provides the floor automatically) but still lie about what the demos need. Audit + remove in a sweep across `crates/rdom-showcase/src/demos/*.rs` + the chrome's source-disclosure block + the Demo trait docstring's authoring admonition. Mechanical change; no behavior shift expected.
