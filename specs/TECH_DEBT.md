...
- **`EVT-DETACH-1` — Implicit focus / hover loss on detach does not fire DOM events.** When the focused element (or any ancestor) is removed from the tree, `dom.focused()` clears synchronously and `Mutation::InteractionChanged { next: None, kind: Focus }` fires — but no `blur` / `focusout` DOM events dispatch through the event pipeline. Same shape for `hovered` clearing (no `mouseleave` / `mouseout`). Documented in [`DIVERGENCES.md`](DIVERGENCES.md) §"Runtime & focus". **Blocking on M5 (event surface bundle).** M5 introduces the runtime's broader hover / motion event model (`mousemove`, `scroll`, `resize`, etc.); the right shape for implicit-detach events is to capture the focused/hovered node's pre-detach ancestor path in `rdom-core::tree::detach_from_parent`, plumb it through `Mutation` (new variant or extended `InteractionChanged` with a `was_detached: bool` flag), and have an `App`-installed observer translate those records into `dispatch_tui_event` calls. Doing this work outside M5 means designing the implicit-event pipeline twice; bundling with M5 means one coherent event-pipeline change. If M5 ships without closing this, escalate then — `EVT-DETACH-1` must close in the same release as `mouseleave` lands or rdom-tui ships an internally inconsistent hover-event model.
