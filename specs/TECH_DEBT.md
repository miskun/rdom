...
- **`CALC-PCT-1` — `calc()` with percentages is not resolved at layout time.** Parse-time constant-eval is shipped (M6): `width: calc(8 * 2)` produces `Size::Fixed(16)` directly. Percent-bearing forms like `width: calc(100% - 4)` parse correctly into a `CalcExpr` AST but are then dropped (return `None` from `parse_size`/`parse_length`) because resolving them requires `Size::Calc(Box<CalcExpr>)` and `Length::Calc(Box<CalcExpr>)` variants plus a layout-pass resolver — work scoped out of 0.2.0 because adding boxed-AST variants removes `Copy` from `Size`/`Length` and ripples `.clone()` through ~50 cascade / layout / animation / pseudo-positioning sites. Pay down with: (a) add the variants and remove `Copy`, (b) add `.clone()` at every move-by-value site, (c) thread `ResolveCtx` through `apply_relative_shift` / `compute_placed_rect` / `layout_flex_children` / `compute_pseudo_layout` / the cascade animation interpolator. The `CalcExpr` substrate (`crates/rdom-style/src/calc.rs`) + parser + resolver are already shipped — only the type-system + per-site `.clone()` work remains.
