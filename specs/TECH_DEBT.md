...
- **`M5-MIN-CONTENT-1` — `min-width: auto` resolves to intrinsic natural size, not strict CSS min-content; `min-height: auto` is not enforced as a floor in the flex shrink pass.** Two axes, one underlying gap. **Width axis:** rdom holds the full text width, CSS allows shrinking to longest-word width with line wrap. **Height axis (M4 discovery):** flex items with explicit `height: N` can shrink below `N` to zero cells under Bresenham allocation when the container overflows — CSS forbids this; `min-height: auto` should floor every item at its intrinsic content height (≥1 line for text-bearing items). This is the root cause of the `sticky_demo` "every Nth item missing" bug; current workaround is author-side `flex-shrink: 0` on fixed-height children, applied across 6 demos in `rdom-showcase`. Pay down by computing intrinsic min-height (text line count, or sum of in-flow children for containers) and clamping the shrink pass against it. **When this closes, the `flex-shrink: 0` declarations across the showcase demos can be removed.**
