...
- **The focus indicator is a typed vocabulary, not a universal outline** (`FOCUS-VOCAB-1`). The web shows focus with an *outline* (a non-destructive ring around the box) on **every** focusable element. A TUI can't draw a no-reflow ring — a border would consume a cell and reflow — so there's no single generic cue. Instead the UA stylesheet matches the affordance to the element kind:
  ...
  - **Scroll containers** (anything that clips and overflows) → the **scrollbar thumb glyph** turns accent (`:focus::scrollbar-thumb { color }` — foreground, a colored handle rather than a filled block); unfocused thumbs are gray. Zero extra area — it reuses chrome the scroll already owns. Such containers are also made implicitly keyboard-focusable (see below) and **keyboard-scrollable** (Arrows / PageUp / PageDown / Home / End / Space, after the editable-key default), matching modern browsers' keyboard-focusable scrollers.
...
- **Scrollable overflow containers are implicitly keyboard-focusable and keyboard-scrollable** (`FOCUS-VOCAB-1`), matching modern browsers. A container qualifies only when it actually shows a scrollbar (clips on an axis *and* content overflows) **and** has no focus stop of its own (no focusable descendant), so it never adds a redundant tab stop around already-reachable content. A non-scrolling `<div>`/`<p>`/`<table>` is not focusable, exactly as on the web. Once focused, Arrows / PageUp / PageDown / Home / End / Space scroll it (the runtime default action, after editable-key handling, `preventDefault`-overridable).
...
- **Active descendant, not roving DOM focus.** The `[role=tree]` container holds focus; the cursor row carries an internal `data-rdom-active` marker and highlights only while the container is focused. The container itself gets no background fill on focus — the focus tint is scoped to atomic controls (`FOCUS-VOCAB-1`), so a tree (or any container) is never flooded; its cue is the active row (and, when scrollable, the accent scrollbar thumb).
