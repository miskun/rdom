...
- **The focus indicator is a background tint, not an outline.** The web draws a focus *outline* (a non-destructive overlay around the box, no reflow). A box outline in a TUI would consume a border cell and reflow content, so the UA stylesheet uses a subtle `:focus { background }` tint instead. Consequences of that substitution, all handled in the UA sheet: (a) **`<canvas>` is exempt** — it's a replaced/content element the app paints, and a background tint would paint over its pixels, which the web's outline never does; a focused canvas keeps its background (apps can still set one explicitly). (b) The generic tint is **non-important** and author-overridable; `input/textarea/select:focus` keeps `!important` only to beat those controls' own high-specificity field background. Authors who want an outline-style focus cue add a border and recolor it on `:focus` (`:focus { border-color: … }`) — already supported; the bg-tint is just the zero-config default. (`UA-FOCUS-OVERRIDABLE-1`.)
