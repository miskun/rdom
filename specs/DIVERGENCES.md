...
- **Cross-text-node edits inside contenteditable** apply as a single `beforeinput` / `input` pair but are **not recorded on the undo stack** — undo would otherwise reverse only one of the affected nodes. Tracked as `EDIT-1`.
