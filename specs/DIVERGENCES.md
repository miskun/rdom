...
- **Painted content occludes the border beneath it (z-aware).** The border joiner runs as a final pass; to keep CSS stacking order (a higher element's content paints *over* a lower element's border, not under it) any glyph painted at a cell clears that cell's accumulated border state. Borders contributed *after* the content paint — higher elements, painted later in stacking order — are untouched, so a higher border still shows over lower content. Mirrors the existing opaque-`background-color` occlusion.
...
- **Custom properties are global and `:root`-only.** `--name: value` is honored only in a rule whose selector is exactly `:root`, where it defines a stylesheet-wide variable; under any other selector (including `:root, body`) it is dropped with an `UnsupportedCustomPropertyScope` warning. There is no per-element custom-property scope or inheritance yet. `var()` is consumed in color positions and in `content`; other property types do not read custom properties.
- **CSS-wide keywords are not accepted on transition properties.** `transition*: inherit | initial | unset` is dropped as invalid (`STYLE-TRANSITION-VALUE-1`); every other property accepts them. (`unset` is resolved at parse time from the inherited-property table — an implementation detail with the same observable result as the web; see `DESIGN.md`.)
...
- **Percentages resolve to whole percent and times to whole milliseconds.** `12.5%` tokenizes intact but `Size::Percent` stores a `u16`, so layout sees 12%; `1.5ms` rounds to 2ms. Cells are integers, so the fraction of a percent rarely changes a result; recorded as `STYLE-PERCENT-FRACTION-1`.
...
- **Pointer capture is mouse-only and does not retarget `event.target`.** `set_pointer_capture` / `release_pointer_capture` capture *the mouse* (rdom has no touch/pen pointers, no `pointerId`). While captured, moves + the `mouseup` route to the owner — but `event.target` stays **the element actually under the pointer** (the hit-tested node), unlike the DOM, which retargets to the capture element. rdom consumers (and root-delegated handlers) want the real position; drag selection reads coordinates, not `target`.
...
- **Event dispatch is synchronous**, as on the web; there is no task queue, only the runtime's `queue_microtask` / timers. Listener order follows DOM §2.9 (capture pass, then bubble pass, with the target in both).
...
- **The focus indicator is a typed vocabulary, not a universal outline** (`FOCUS-VOCAB-1`). The web shows focus with an *outline* (a non-destructive ring around the box) on **every** focusable element. A TUI can't draw a no-reflow ring — a border would consume a cell and reflow — so there's no single generic cue. Instead the UA stylesheet matches the affordance to the element kind:
  ...
  - **Scroll containers** (anything that clips and overflows) → the **scrollbar thumb glyph** of the scroll region *containing the focus* turns accent (`:focus-within::scrollbar-thumb { color }` — foreground, a colored handle rather than a filled block); other thumbs are gray. This marks the region the keyboard scroll keys act on. Zero extra area — it reuses chrome the scroll already owns. Such regions are also keyboard-focusable when they're the focus target (see below) and keyboard-scrollable from wherever the focus is. **Known limitation (`FOCUS-THUMB-NEAREST-1`):** `:focus-within` matches *every* scroll ancestor of the focus, so if two nested scroll containers both overflow at once, both thumbs colorize even though the keyboard scrolls only the nearest. Rare in practice (usually one ancestor overflows); a post-layout "nearest scroll container of focus" marker would make the cue exact — see `TECH_DEBT.md`.
...
- **Scrollable overflow containers are implicitly keyboard-focusable** (`FOCUS-VOCAB-1`), matching modern browsers. A container qualifies only when it actually shows a scrollbar (clips on an axis *and* content overflows) **and** has no focus stop of its own (no focusable descendant), so it never adds a redundant tab stop around already-reachable content. A non-scrolling `<div>`/`<p>`/`<table>` is not focusable, exactly as on the web.
- **Keyboard scroll keys act on the nearest scrollable ancestor of the focused element** (Arrows / PageUp / PageDown / Home / End / Space). So `PageDown` while focused on an `<input>` pages the form's scroll pane, and a focused scroll container scrolls itself. The runtime default action runs after editable-key handling (a focused field still moves its caret) and after widget activation (`<button>`, `[role=tree]` claim their keys via `preventDefault`); it's `preventDefault`-overridable.
