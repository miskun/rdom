...
- **`resize` dispatches on the document root, not on `Window`.** rdom has no Window object (terminals don't have window-level chrome to model). The event-target divergence is purely about the target identity; bubbling, the cancelable flag, and the timing of dispatch all match the HTML spec for `Window.resize`. Listen on `dom.root()` to handle terminal resize.
