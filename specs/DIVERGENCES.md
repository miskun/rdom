...
- **`min-width: auto` resolves to intrinsic natural size**, not strict CSS min-content (longest-word width).
- **Explicit `min-*: auto` is more protective than CSS strict.** Per CSS Flexbox §4.5, an explicit `min-width: auto` (or `min-height: auto`) on a flex item resolves to `min(content_size_suggestion, specified_size_suggestion)` — so `flex: 1; min-width: auto` collapses to 0 (because `flex: 1` sets flex-basis to 0%). rdom treats explicit `min-*: auto` as a content-protection opt-in: the floor is the content size suggestion regardless of the specified cap, so `flex: 1; min-width: auto` keeps the item at least at its intrinsic content. Implicit min (`min` unset, the spec default for flex items) still follows strict CSS — so `flex: 1` alone allows full shrink-to-zero. Authors who want strict CSS behavior write `min-width: 0` explicitly; authors who want "protect my content" write `min-width: auto`. The implicit-vs-explicit asymmetry is the rdom flavor.
...
- **`display: inline-block` siblings disqualify the parent from IFC.** CSS treats inline-block as an inline-level box that participates in IFC alongside other inline content; rdom routes inline-block children through flex layout instead. When a container has at least one inline-block child, the IFC predicate returns false and the parent uses flex layout for all children. This is closer to CSS Flexbox §3 step 7 (which blockifies a flex container's children) and avoids the IFC paint path dropping the inline-block's UA pseudos (`<button>`'s `[ ... ]` brackets). Mixed text-and-inline-block content inside the same container is unsupported as a result — tracked as `IFC-MIXED-TEXT-INLINEBLOCK-1` in `TECH_DEBT.md`.
