...
- **`border-style: half-block` is rdom-specific.** Not a CSS-spec keyword. Paints each border cell with a half-block (`▄ ▀ ▌ ▐`, U+2584/U+2580/U+258C/U+2590) or quadrant glyph (`▗ ▖ ▝ ▘`, U+2596/U+2597/U+259D/U+2598) whose filled half-or-quarter points inward toward the bordered element's content. Pairs with a `background-color`-filled interior to produce a "pill"-style primary-CTA button that visually reads as ~2 cells tall on a 3-row layout (the half-blocks contribute half-cells of color each, joining the filled interior into a continuous accent region). Use only for the lone-element case — when a half-block border shares a cell with another bordered element (T-junction, cross), the joiner emits no glyph because there's no defined half-block analog for those mask patterns.
