...
- **The focus indicator is a typed vocabulary, not a universal outline** (`FOCUS-VOCAB-1`). The web shows focus with an *outline* (a non-destructive ring around the box) on **every** focusable element. A TUI can't draw a no-reflow ring — a border would consume a cell and reflow — so there's no single generic cue. Instead the UA stylesheet matches the affordance to the element kind:
  ...
  - **Scroll containers** (anything that clips and overflows) → the **scrollbar thumb** turns accent (`:focus::scrollbar-thumb { background }`); unfocused thumbs are gray. Zero extra area — it reuses chrome the scroll already owns. Such containers are also made implicitly keyboard-focusable (see below), matching modern browsers' keyboard-focusable scrollers.
...
- **Scrollable overflow containers are implicitly keyboard-focusable** (`FOCUS-VOCAB-1`), so they can be scrolled from the keyboard — matching modern browsers. A container qualifies only when it actually shows a scrollbar (clips on an axis *and* content overflows) **and** has no focus stop of its own (no focusable descendant), so it never adds a redundant tab stop around already-reachable content. A non-scrolling `<div>`/`<p>`/`<table>` is not focusable, exactly as on the web.
