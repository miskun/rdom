...
- **`unset` is resolved at parse time** into `inherit` or `initial` from the style crate's inherited-property table (`property_dispatch::inherits`), rather than carried to the cascade. Same result as the web for every property rdom inherits.
