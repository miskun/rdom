...
- **Implicit focus loss on detach does not fire `blur`.** When the focused element (or an ancestor of it) is removed from the tree, `dom.focused()` synchronously clears — matching the web — but only a `Mutation::InteractionChanged { next: None, kind: Focus }` record fires. The browser additionally fires a `blur` event on the detached element and a `focusout` that bubbles to body; rdom emits no events for *implicit* focus loss (explicit `set_focused(Some(_))` while a different node is focused still fires the focus/blur events). The same applies to `hovered` clearing on detach (no synthetic `mouseleave`) and to `selection` collapsing on detach (no `selectionchange`-with-removed-node event — the cleared selection just stops appearing in the next paint). Pay down by routing the affected dispatches through the runtime's event pipeline when the runtime's listener model can accept synthetic dispatches keyed on a node that's already detached.
