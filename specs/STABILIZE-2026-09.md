...
- 2026-09-26 — Phase 7: `P7-VALIDATION-SELECTORS-1` — HTML §4.16.3 form pseudo-classes. rdom-core: `PseudoClass::{Valid, Invalid, Required, Optional}`; `:required` / `:optional` are attribute + type-state logic (`Dom::is_required_control` / `is_optional_control`); `:valid` / `:invalid` go through `Dom::constraint_validity` — candidates by a backend `ValidityHook` (`fn(&Dom<Ext>, NodeId) -> bool`, `Dom::set_validity_hook`; valid without one), forms by their owned candidates, fieldsets by descendant candidates. Chosen over reflecting validity into an internal attribute: matching stays pull-based, so `matches` / `query_selector` are always current and nothing leaks into attributes, serialization or observers; rdom-core stays dependency-free (the regex lives behind the hook in rdom-tui). rdom-tui: `validation::install` (called in `App` construction) installs the hook; `ValidityMarks` (`validation/marks.rs`) keeps the incremental cascade current — before each frame it recomputes the state of every candidate / form / fieldset and marks the flipped ones dirty (textarea text edits and `set_custom_validity` fire no cascade-dirtying record; a radio group's siblings and ancestor forms / fieldsets are not the mutated node), only while some sheet uses `:valid` / `:invalid`; the UA sheet does not (browsers do not style `:invalid`, only the bubble). rdom-style specificity counts them as pseudo-classes. Red → green: `required_and_optional_match_the_three_form_controls`, `valid_and_invalid_follow_the_validity_hook` (compile), `installed_hook_drives_valid_and_invalid` (compile), `validity_changes_restyle_on_the_next_frame` (before the App installed the hook: `left: Reset, right: Rgb(255, 0, 0)`; before the marks were flushed: `textarea edit restyles` failed), `the_ua_sheet_has_no_validity_rules`. No expectation changed. DIVERGENCES: the four pseudo-classes move to the supported list; §3 keeps `:user-valid` / `:user-invalid` and the unrendered input types. README / DESIGN open-polish mention of form validation removed.
- 2026-09-26 — Phase 7: `P7-RADIO-GROUP-OWNER-1` — HTML §4.10.5.1.18 radio button group. New rdom-core `radio_group.rs`: `Dom::radio_group` (tree order, one pass, form owners resolved only for same-named radios) and `Dom::in_same_radio_group`. rdom-tui routes exclusivity (`toggle::pre_activate`), arrow-key movement, the single Tab stop (`focus::tabindex::dedupe_radio_groups`, now a linear scan of group representatives) and `valueMissing` (`validation::states`) through it; the name-keyed `toggle::radio_group` is gone. Red → green: `same_named_radios_in_two_forms_toggle_independently`, `a_radio_outside_a_form_with_a_form_attribute_groups_with_that_form` (`form=f puts the radio in f's group`), `same_named_radio_groups_in_two_forms_each_contribute_a_tab_stop` (`left: [NodeId(4)] right: [NodeId(4), NodeId(6)]`), `required_radio_validity_follows_the_form_owner_group`; new: `arrow_navigation_stays_inside_the_form_owners_group`, `radio_names_that_differ_in_case_are_different_groups` (pins the exact comparison, green before), four `radio_group::tests` in rdom-core. No expectation changed. DIVERGENCES §2 name-keyed radio-group entry removed.
