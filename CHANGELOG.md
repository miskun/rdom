...
- `compute_content_area_collapsed` takes a fifth argument, `containing_block_width`, the basis for percent padding; `compute_content_area` keeps its three-argument form and resolves against the box's own width. (`CALC-PADMARG-1`)
...
- The focus scrollbar-thumb cue marks exactly the scroll container the keyboard scrolls. The runtime keeps `data-rdom-scroll-focus` on the nearest overflowing scroll ancestor of the focus and the UA rule is `[data-rdom-scroll-focus]::scrollbar-thumb` (was `:focus-within::scrollbar-thumb`, which lit every overflowing ancestor). (`FOCUS-THUMB-NEAREST-1`)
...
- The margin-collapse chain walkers memoize per block within a layout pass (`TuiExt::margin_chain`, `MarginChainMemo`): placing an ancestor records the chain result of every level it walks, each level consumes its own entry when placed, and `layout_node` clears it, so a deep first-child chain is walked once instead of once per level. Entries are keyed by containing-block width. The `block_layout` bench gains a 60-level chain shape next to the 20-level one. (`BFC1-PERF-MARGIN-CHAIN-1`)
