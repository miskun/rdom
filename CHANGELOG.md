...
- UA: `<input type=checkbox|radio>` no longer inherit the text field's `width: 20`; they hug their glyph (4 cells) in a flex row. (`FLEX-BLOCK-MAIN-INTRINSIC-1`)
- UA: the focus scrollbar-thumb rule is `[data-rdom-scroll-focus]::scrollbar-thumb` (was `:focus-within::scrollbar-thumb`, which lit every overflowing ancestor); the rdom-tui runtime keeps that attribute on the scroll container the keyboard scrolls (`SCROLL_FOCUS_ATTR`, public). An author override of the old selector must move to the new one. (`FOCUS-THUMB-NEAREST-1`)
...
- Paint and hit-testing share one stacking walk (`render::stacking`) and the layout pass's in-flow filter instead of three private copies. (`DRY-1`, `DRY-2`)
- An absolutely or fixed positioned element with both insets of an axis `auto` sits at its static position (CSS 2.1 §10.3.7 / §10.6.4): after the preceding block sibling and its collapsed margin, after the preceding text for an inline-level box, on the next line for a block-level one, and at the content start of a flex container. Phase-1 layout records it in the new `TuiExt::static_position` (`StaticPosition`), a read-only layout output like `layout`. (`D-M2-2`)
- **Stacking contexts (CSS 2.1 Appendix E).** Paint and hit-test nest: a positioned element with a numeric `z-index` (or any element with `opacity < 1`) forms a stacking context whose descendants compete only inside it; `z-index: auto` boxes hand their positioned descendants to the enclosing context; negative `z-index` paints below the context's in-flow content and above its background. `position: relative` and `sticky` boxes paint in the positioned layer, above the in-flow siblings they overlap (a pinned sticky header now stays above the rows that scroll under it). An absolutely positioned box is clipped by an overflow ancestor only when its containing block is that ancestor or inside it (CSS 2.1 §11.1.1) — previously every positioned box escaped every overflow clip. The shared walk lives in `render::stacking`. (`D-M2-3`, `D-M2-4`)
...
- `inline_flow_for_text` resolves an anonymous block box through its `child_range` instead of scanning every fragment of every box: O(ancestors × siblings) per caret move instead of O(anonymous boxes × lines × fragments). A whitespace-only inline run now resolves to its own (empty) box. (`BFC1-PERF-INLINE-FLOW-LOOKUP-1`)
- An `auto`-height block counts its scrollbar gutter row: pass 2 of the classic-scrollbar layout re-resolves the height after reserving the gutter (for text leaves and inline formatting contexts too), and intrinsic sizing counts a permanent (`overflow: scroll` / `scrollbar-gutter: stable`) gutter as a row or column of the box. Previously the gutter ate the last content row. The two-pass design and the auto-height ordering are now recorded in `DESIGN.md`. (`SCROLLBAR-AUTO-TWO-PASS-1`, `BFC1-AUTO-HEIGHT-ORDERING-1`)
...
- SGR emission no longer allocates per changed cell: the modifier codes come from a const table. (`SGR-ALLOC-1`)
- Paint borrows each element's inline layout and anonymous boxes instead of cloning them per frame. (`PAINT-INLINE-LAYOUT-CLONE-1`)
- The line packer borrows graphemes from the text node instead of allocating a `String` per grapheme; one `String` per emitted fragment remains. (`PACKER-STRING-ALLOC-1`)
...
- A block's static `::before` / `::after` paint on inline formatting contexts and on mixed content (the first / last anonymous box when the first / last in-flow child is inline-level); they used to paint only on pure-text blocks. One line painter serves all three. (`TREE-BFC-PSEUDO-1`)
...
- A drag leaving a `user-select: contain` host clamps to the in-host line nearest the pointer's row (past its end when the pointer is beside it), no longer to the host's first or last text end. The host of `user-select: contain` / `all` is the element that declared it, not a descendant — a click in the second paragraph of an `all` host now selects the whole host (see `P6G-USER-SELECT-INHERIT-1` for how hosts resolve). (`EDIT-2`)
- `<form>` reset restores text inputs, textareas, checkboxes and radios: `defaultValue` / `defaultChecked` are captured when a control is seeded or before its first change (`TuiExt::default_value` / `default_checked`, `default_value()` / `default_checked()` accessors) and reset writes them back — text content plus the mirrored `value` attribute, and the `checked` attribute — without firing `input` / `change`. The attributes keep reflecting the live state (see DIVERGENCES). `<select>` and `<input type=range>` joined in `P6G-FORM-RESET-1`. (`FORM-DEFAULTS-1`)
- **`opacity` is group opacity.** A translucent element's subtree paints into a layer at full opacity and composites back once, so nested opacities multiply (`0.5` in `0.5` renders at `0.25`) and pseudo-element backgrounds no longer double-blend; a translucent box still leaves the glyphs beneath it visible. The per-write compose context on `Buffer` is gone (`enter_compose_ctx` / `exit_compose_ctx` / `compose_bg_for_cell` were crate-private). (`OPACITY-1`)
- `layout_pass/flex.rs` is now `flex/{mod,main_axis,cross,placement,collapse}.rs`: orchestration, the §9.7 grow / shrink freeze loops and auto-min resolution, cross sizing and alignment, main-axis placement, and the collapsed-border sibling overlap predicate that sizing and placement used to duplicate. No behavior change. (`FLEX-RS-SPLIT-1`)
- `runtime/scrollbar/mod.rs` is now `scrollbar/{mod,hit,drag,keys,autoscroll,reveal,geometry,scroll}.rs`: hit testing, the thumb drag session, keyboard scrolling, edge autoscroll, caret / rect reveal, scroll-container geometry, and the single scroll writer. Crate-internal `nearest_scroll_container` (no callers) narrowed to the module. No behavior change. (`SCROLLBAR-SPLIT-1`)
- `runtime/hit_test/mod.rs` is now `hit_test/{mod,descend,nearest,fragment}.rs`: the `HitTestExt` surface, the stacking-context descent with pointer-events resolution, the nearest-inline-target fallback, and fragment-to-position resolution. No behavior change. (`HIT-TEST-SPLIT-1`)
- `builtins/select/mod.rs` is now `select/{mod,model,state,click,keyboard,typeahead,dropdown}.rs`: the option model, the shared selection writers, mouse activation, keyboard navigation, type-ahead, and dropdown open / close state; the two root listeners are named functions. Stale module docs (claims of no dropdown and no type-ahead) corrected. No behavior change. (`SELECT-SPLIT-1`)
- `runtime/app/mod.rs` is now `app/{mod,stylesheets,keyboard_defaults,autoscroll,frame}.rs`: the struct, construction and event loop stay in `mod.rs`; the stylesheet stack and intents, the clipboard / undo / editable key defaults, the drag-autoscroll session, and the frame (`draw_if_dirty`, cascade + layout, caret-reveal servicing, scroll-focus marking) each own a file. Public API unchanged. No behavior change. (`APP-MOD-SPLIT-1`)
- `paint_pass/inline_paint.rs` is now `inline_paint/{mod,single_row,caret,selection_overlay,chrome}.rs`, and the paint pass no longer knows builtin internals: `<select>` display text, password masking and gauge chrome come through one seam, `ChromeText` / `InlineChromeFn` supplied by `runtime::builtins::inline_chrome`. The supplier table is compiled in rather than registered per `Dom`, so a bare `paint_dom` (no `App`) still paints chrome. No behavior change. (`INLINE-PAINT-SPLIT-1`)
- `rdom-style`'s `property_dispatch.rs` is now `property_dispatch/{mod,table,css_wide,set,serialize,value_serializers}.rs`: the property ↔ field table, the `inherit` / `initial` / `unset` keywords, `set`, `serialize`, and the per-value serializers. Public paths unchanged. No behavior change. (`STYLE-DISPATCH-SPLIT-1`)
- `rdom-style`'s `parse/values.rs` is now `parse/values/{mod,color,keyword,number,length,spacing,border,content,transition,calc}.rs`, one file per value family; the `calc()` parser sits with its entry point and the evaluator stays in the crate-root `calc.rs`. Public paths unchanged. No behavior change. (`STYLE-VALUES-SPLIT-1`)
- `cssom/declaration.rs` (~1190 lines) is now a directory module split by concern: `error` (`SetPropertyError`), `read` (`StyleDeclaration`), `write` (`StyleDeclarationMut`), `serialize` (`cssText` + shorthand suppression), and tests grouped by read / write / cssText. No behavior or public-path change. (`DECLARATION-SPLIT-1`)
- `render/buffer.rs` (~1090 lines) is now a directory module split by concern: the grid + construction + indexing + bulk operations in `mod`, the border-direction / half-block side tables in `border`, cell and string writes in `write`, group compositing in `composite`, the frame diff in `diff`, and `tests`. No behavior or public-path change. (`BUFFER-SPLIT-1`)
- **Breaking:** `VirtualScreen` (`rdom_tui::VirtualScreen`, `rdom_tui::render::VirtualScreen`, `rdom_tui::render::virtual_screen`, and its prelude re-export), the test-only VT emulator, is no longer part of the default API — it is behind the new `test-util` feature (add `features = ["test-util"]` to the `rdom-tui` dev-dependency). The module is also split into `virtual_screen/{mod,parse}.rs` (screen model vs. ANSI parser) with its tests grouped by parser / real-terminal replay. (`VIRTUAL-SCREEN-TEST-UTIL-1`)
- `colspan` is honored by the table column pass: a spanning cell takes the sum of its columns' used widths, content wider than that sum grows the spanned columns evenly (CSS 2.2 §17.5.2.2), and the following cells land in the slots after the span. `rowspan` stays unimplemented — the table model (flex rows + column pre-pass, no Table Formatting Context) is now recorded as a deliberate divergence in `DIVERGENCES.md`. (`TABLE-COLSPAN-1`, `TABLE-TFC-1`)
- The margin-collapse chain walkers memoize per block within a layout pass (crate-internal `TuiExt::margin_chain`): placing an ancestor records the chain result of every level it walks, each level consumes its own entry when placed, and `layout_node` clears it, so a deep first-child chain is walked once instead of once per level. Entries are keyed by containing-block width. (`BFC1-PERF-MARGIN-CHAIN-1`)
- Internal: `layout_pass/block.rs` is split into `block/{mod, margin_collapse, width, height}.rs` (placement loop; §8.3.1 margin collapsing; §10.3.3 widths; §10.5 / §10.6.3 heights). No behavior change. (`BFC1-CODE-BLOCK-SPLIT-1`)
...
- UA sheet: `input[type=checkbox]`, `input[type=radio]` and `input[type=range]` no longer declare `user-select: none` (browsers' UA sheets do not); a click on them beside prose reaches them because the selection drag takes no pointer capture. The button family keeps its UA `user-select: none`, now listed in DIVERGENCES. (`P6G-TOGGLE-USER-SELECT-REVERT-1`)
- **Behavior change:** `user-select` is no longer inherited (CSS UI 4 §6.1) — a child of a `none` element now *computes* `auto`, and `unset` means `initial`. The runtime resolves the used value instead: `auto` takes `none` / `all` from its parent, is `contain` on a text control or `contenteditable` host, and is `text` otherwise, so an explicit `text` (or `contain`) descendant of a `none` chrome bar is selectable, highlighted and copied, a nested `contain` element is its own host (a drag started in it no longer escapes into an outer `contain` host), and the `all` host is the outermost `all` ancestor. A copy whose end sits in `none` text stops there. (`P6G-USER-SELECT-INHERIT-1`)
- `<form>` reset now also restores `<select>` (each option to its `defaultSelected`, captured for every option before the select's first change — new `TuiExt::default_selected`) and `<input type=range>` (its authored `value` attribute, removed again when there was none). `TuiAccessorsMut::set_value` on a `<textarea>` or range keeps the authored default; `default_value()` returns the `value` attribute for non-text inputs (`"30"` for `<input type=range value=30>`, was `""`) and toggles (was `None`); new `TuiAccessorsMut::set_default_value` / `set_default_checked` (required trait methods) set what a reset restores without touching the live state. (`P6G-FORM-RESET-1`)
- A static `::before` / `::after` is now laid out by the line packer as the host's first / last inline content (CSS 2.1 §12.1) instead of being shifted in at paint time: text wraps around it instead of overflowing and clipping, and the caret, clicks and the selection highlight all agree with what is painted. New public API: `LineBox::generated` (`Vec<GeneratedFragment>`) carries the generated runs; they are never `InlineFragment`s, so they have no DOM position (`P6G-PSEUDO-SHIFT-1`)
- A `::before` whose host starts with a block-level child (and an `::after` whose host ends with one) now renders on a line of its own (CSS 2.1 §9.2.1.1) instead of being dropped; an `<li>`'s `::before` acts as its list marker and rides the first line of its block child, so `<ol><li><p>Step</p></li></ol>` shows "1. Step" and `ul > li > p` shows its bullet (DIVERGENCES: list markers) (`P6G-BLOCK-FIRST-PSEUDO-1`)
- `::before` / `::after` on inline elements inside an inline formatting context (`b::before { content: "[" }` in `<p>a <b>bold</b> c</p>`) now render, in the pseudo's style, at the element's start / end: they wrap with the text, count toward max-content width, resolve `counter()`, carry an enclosing `<a href>`'s OSC 8 link, and a click on one targets its host; caret and click offsets stay on the text (`P6G-INLINE-PSEUDO-1`)
...
- `<select>` now runs HTML's selectedness setting algorithm: a single-select dropdown with no `selected` option selects its first non-disabled option (was value `""`), and keeps only the last of several `selected` options — at mount, after a `<form>` reset, and (settled before the App's next event or frame) after options are inserted or removed; a consumer marking one option `selected` makes it the only one. The algorithm's pick is not a default. New public API: `TuiAccessors::default_selected` / `TuiAccessorsMut::set_default_selected` on `<option>` (required trait methods) and `select::seed_all` for DOMs run without an `App` (`P6G-SELECT-SELECTEDNESS-1`)
