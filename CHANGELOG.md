...
- `<form>` reset restores every control: `defaultValue` / `defaultChecked` are captured when a control is seeded or before its first change (`TuiExt::default_value` / `default_checked`, `default_value()` / `default_checked()` accessors) and reset writes them back — text content plus the mirrored `value` attribute, and the `checked` attribute — without firing `input` / `change`. The attributes keep reflecting the live state (see DIVERGENCES). (`FORM-DEFAULTS-1`)
