...
- A drag leaving a `user-select: contain` host clamps to the in-host line nearest the pointer's row (past its end when the pointer is beside it), no longer to the host's first or last text end. The host of `user-select: contain` / `all` is the element that declared it (the outermost of the run of ancestors computing that value), not the innermost descendant inheriting it — a click in the second paragraph of an `all` host now selects the whole host. (`EDIT-2`)
...
- UA sheet: `input[type=checkbox]`, `input[type=radio]` and `input[type=range]` are `user-select: none`, like the button family. A mousedown on a toggle beside prose used to fall through the empty-space snap to the nearest text, start a selection drag there, and the drag's pointer capture retargeted the click away from the toggle — it never flipped. (`FORM-DEFAULTS-1`)
